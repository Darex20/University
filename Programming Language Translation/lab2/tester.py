...
test = "./test2/";		# folder that contains .in and .out files in itself or its subfolders

using_python = True;				# did you write lab in python
if not using_python:
    exe  = './SintaksniAnalizator.py';	# path to .exe    can be anything if     using_python
    exe = os.path.abspath(exe);
    args = [exe];
else:
    exe  = './SintaksniAnalizator.py';		# path to .py     can be anything if not using_python
    exe = os.path.abspath(exe);
    args = ['py', exe];
pass;
